...
\begin{proof}
    ...
    \end{align}
    Comparing the diagonal entries, we get $\lambda_{j}^{2} = \abs{\lambda_{j}}^{2} + \sum_{i < j} \abs{t_{ij}}^{2}$ which shows that the $\lambda_{i}$'s are real, which again shows that $t_{ij} = 0$ for all $i < j$. Thus, $T$ is diagonal, and $A^{\ast} = A$.
\end{proof}
