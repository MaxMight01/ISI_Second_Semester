...
\end{theorem}
\begin{proof}
    Suppose $B$ and $C$ are congruent, that is, they represent the same quadratic form over different bases of $\R^{n}$. Let $(v_{1},\ldots,v_{n})$ be a basis of $\R^{n}$ consisting of orthonormal eigenvectors of $B$. By spectral theorem, this is possible since $B$ is symmetric. Similarly, let $(w_{1},\ldots,w_{n})$ be a basis of $\R^{n}$ consisting of orthonormal eigenvectors of $C$. Let $Bv_{i} = a_{i}v_{i}$ and $Cw_{j}=b_{j}w_{j}$. We rearrange the basis in a way so that the vectors corresponding to the positive eigenvalues occur first, the vectors corresponding to the negative eigenvalues occur next, and the vectors corresponding to eigenvalue 0 occur last.
